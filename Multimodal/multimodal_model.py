...
class TabVis(nn.Module):
    ...
    def __init__(self, tab_model, vis_model, num_classes=1): 
        ...
        self.mixed_reg = nn.Sequential(nn.Linear(1124,1124),
                                       nn.ReLU(inplace=True),
                                       nn.Linear(1124, num_classes))
        
        # receive the weights from tab and spectral modules
        self.tab_reg = nn.Linear(100, num_classes)
        self.vis_reg = nn.Linear(1024, num_classes)
        ...
        self.vis_handle = self.vis_model.classifier.register_forward_hook(get_vis_logits)
